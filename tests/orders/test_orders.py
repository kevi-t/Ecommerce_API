...
class OrdersTests(TestCase):
    ...
    def test_create_order_success(self):
        with patch('orders.views.send_sms') as mock_sms:
    ...
    def test_create_order_legacy_url_success(self):
        with patch('orders.views.send_sms') as mock_sms:
            mock_sms.return_value = {'status': 'success'}

            response = self.client.post(
                '/api/ecommerce/order/place-order/',
                self.valid_order_data,
                format='json',
            )
    ...
    def test_create_order_uses_authenticated_customer(self):
        other_customer = Customer.objects.create_user(
            email='other-create@example.com',
            name='Other User',
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Orders.objects.get().customer, self.customer)
    ...
    def test_list_customer_orders_legacy_url(self):
        Orders.objects.create(customer=self.customer, item='Phone', amount='500.00')

        response = self.client.get('/api/ecommerce/order/list/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.json()), 1)
        self.assertEqual(response.json()[0]['item'], 'Phone')
        self.assertEqual(str(response.json()[0]['amount']), '500.00')
